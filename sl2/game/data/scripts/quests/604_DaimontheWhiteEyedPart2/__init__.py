...
class Quest (JQuest) :
 ...
 def onAdvEvent (self, event, npc, player) :
   if event == "Daimon the White-Eyed has despawned" :
      ...
      npc.broadcastPacket(NpcSay(npc.getObjectId(),0,npc.getNpcId(),"Darkness could not have ray?"))
      self.addSpawn(31541,186304,-43744,-3193,57000, False, 0, True)
      return
   ...
   if event == "31683-02.htm" :
      if st.getPlayer().getLevel() < 73 :
       ...
       if st.getQuestItemsCount(S_SUMMON) == 0 :
         ...
         spawnId.broadcastPacket(NpcSay(npc.getObjectId(),0,npc.getNpcId(),"Who called me?"))
