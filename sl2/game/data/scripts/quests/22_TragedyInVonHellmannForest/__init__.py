   ...
   if event == "31334-02.htm" :
       ...
       if st.getInt("id") == 2 and st.getQuestItemsCount(CROSS) > 0 and st.getQuestItemsCount(SKULL) > 0 :
           if self.tifaren == 1 :
               ...
               priest.broadcastPacket(NpcSay(npc.getObjectId(),0,npc.getNpcId(),"Did you call me, "+player.getName()+"?"))
       ...
       if self.well == 0 :
           ...
           soul.getAI().setIntention(CtrlIntention.ATTACK,player,None)
       else :
       ...
       npc.broadcastPacket(NpcSay(npc.getObjectId(),0,npc.getNpcId(),"I'm confused! Maybe it's time to go back."))
       ...
       npc.reduceCurrentHp(9999999,npc,None)
       self.tifaren = 0
       npc.broadcastPacket(NpcSay(npc.getObjectId(),0,npc.getNpcId(),"IMy train of thought is chaotic. It goes back to the beginning of time..."))
