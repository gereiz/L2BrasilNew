...
DROPLIST={ARANEID:[INCENSE_POUCH,30,GEM_OF_MAILLE,"5"],
          M_LIZARDMAN_GUARD:[GEM_OF_MAILLE,30,INCENSE_POUCH,"5"],
          M_LIZARDMAN_SCOUT:[GEM_OF_MAILLE,30,INCENSE_POUCH,"5"]
}
#REWARDS
   ...
   if st.getState() != State.COMPLETED :
    if event == "30334-1.htm" and cond == 0 :
     ...
     if st.getQuestItemsCount(INCENSE_POUCH) == st.getQuestItemsCount(GEM_OF_MAILLE) == 30 and cond == 5 :
       ...
       st.rewardItems(GREEN_COLORED_LURE_HG,60)
       st.giveItems(BABY_DUCK_RODE,1)
       st.rewardItems(FISHING_SHOT_NG,500)
