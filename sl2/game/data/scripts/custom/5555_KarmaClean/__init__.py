...
QuestId     = 5555
QuestName   = "CleanKarma"
QuestDesc   = "custom"
InitialHtml = "1.htm"

print "Clean Karma Enabled..."
...
class Quest (JQuest) :
 ...
 def onAdvEvent (self,event,npc,player) :
    ...
    if event == "Reduce" :
      if st.player.getKarma() > 0 :	
       if st.getQuestItemsCount(PriceID) >= PriceCount :	 
                 st.player.setKarma(0)			  
                 st.getPlayer().broadcastUserInfo();			  
                 st.playSound("ItemSound.quest_finish")			  
                 st.player.sendMessage("Congratulations, you reduced You Karma to 0!")			  
