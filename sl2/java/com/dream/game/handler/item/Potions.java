...
import com.dream.game.network.serverpackets.UseSharedGroupItem;
...
public class Potions implements IItemHandler
{
	...
	private static boolean isEffectReplaceable(L2Playable playable, L2EffectType effectType, L2ItemInstance item)
	{
			...
			if (e.getEffectType() == effectType)
				...
				if (e.getSkill().isPotion())
				{
					if (e.getElapsedTaskTime() > e.getTotalTaskTime() * 2 / 3)
						return true;
					activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_PREPARED_FOR_REUSE).addItemName(item));
					return false;
				}
	...
	private static boolean isUseable(L2Playable playable, L2EffectType effectType, L2ItemInstance item, int skillid)
	{
		return isEffectReplaceable(playable, effectType, item) && isUseable(playable, item, skillid);
	}

	private static boolean isUseable(L2Playable playable, L2ItemInstance item, int skillid)
	{
		L2PcInstance activeChar = playable instanceof L2PcInstance ? (L2PcInstance) playable : ((L2Summon) playable).getOwner();
	...
	private static void itemNotForPets(L2PcInstance activeChar)
	{
		...
		switch (itemId)
		{
			...
			case 726: // mana drug, xml: 2003
				if (!Config.ALT_MANA_POTIONS)
					return;
				res = usePotion(playable, 2003, 1, item); // configurable through
				// xml
				break;
	...
	public boolean usePotion(L2Playable activeChar, int magicId, int level, L2ItemInstance item)
	{
		...
		if (skill != null)
		{
			...
			{
				activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_PREPARED_FOR_REUSE).addSkillName(skill));
				return false;
			}
			...
			{
				((L2PetInstance) activeChar).getOwner().sendPacket(SystemMessage.getSystemMessage(SystemMessageId.PET_USES_S1).addString(skill.getName()));
				return true;
			}
		}
		return false;
