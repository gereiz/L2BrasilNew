...
public class AioItem3 implements IItemHandler
{
	...
	public synchronized void useItem(L2Playable playable, L2ItemInstance item)
	{
		...
		else
		{
			...
			else if (playable.destroyItem("Consume", item.getObjectId(), 1, null, false))
			{
				...
				player.setAio(true);
				player.setEndTime("aio", Config.AIO_DIAS3);
				...
				player.sendMessage("Dear " + player.getName() + " now you have AIOx Status " + " you have acess to all Buff Skill's," + " remember you have " + Config.AIO_DIAS3 + " AIOx days.");
