...
abstract class ConditionLogic extends Condition
{
	...
	{
		if (condition == null)
			throw new IllegalStateException("Tried to add a 'null' condition to an <" + getClass().getSimpleName().replace("ConditionLogic", "").toLowerCase() + "> condition");
	...
	public final Condition getCanonicalCondition()
	{
		if (_conditions.length == 0)
			throw new IllegalStateException("Empty <" + getClass().getSimpleName().replace("ConditionLogic", "").toLowerCase() + "> condition");
