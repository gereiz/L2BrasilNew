...
public class DevastatedCastleSiege extends ClanHallSiege
{
	...
	{
		_log.info("Clan Hall Siege: Devasted Castle Initialized.");
		if (_instance == null)
		{
			_instance = new DevastatedCastleSiege();
		}
	...
	private final List<L2SiegeClan> _registeredClans = new ArrayList<>(); // L2SiegeClan
	...
	private final ExclusiveTask _endSiegeTask = new ExclusiveTask()
	{
		...
		protected void onElapsed()
		{
			...
			if (3600000 > timeRemaining)
				...
				{
					announceToPlayer(Math.round(timeRemaining / 60000.0) + " before the end of the siege Clan Hall " + _clanhall.getName() + ".");
				}
				else
				{
	...
	private final ExclusiveTask _startSiegeTask = new ExclusiveTask()
	{
		...
		protected void onElapsed()
		{
			...
			if (86400000 > timeRemaining)
			{
				...
				{
					announceToPlayer("Siege Clan Hall: " + _clanhall.getName() + " will come in " + Math.round(timeRemaining / 1000.0) + " seconds!");
				}
			}
			int divider;
