...
public class Lottery
{
	private class finishLottery implements Runnable
	{
		...
		public void run()
		{
			...
			if (count1 > 0)
			{
				Announcements.getInstance().announceToAll(SystemMessage.getSystemMessage(SystemMessageId.AMOUNT_FOR_WINNER_S1_IS_S2_ADENA_WE_HAVE_S3_PRIZE_WINNER).addNumber(getId()).addNumber(getPrize()).addNumber(count1));
			}
			else
			{
				Announcements.getInstance().announceToAll(SystemMessage.getSystemMessage(SystemMessageId.AMOUNT_FOR_LOTTERY_S1_IS_S2_ADENA_NO_WINNER).addNumber(getId()).addNumber(getPrize()));
			}
			con = null;
			try
			{
	...
	private class startLottery implements Runnable
	{
		...
		private void announceLottery()
		{
			...
			Announcements.getInstance().announceToAll("On sale-lottery tickets " + getId() + "Matchday Lotteries good luck.");
		}
