...
public class L2PcOffline extends Thread
{
	...
	{
		_log.info("Offline Manager: Restore traders disabled.");
		Connection con = null;
		try
		{
	...
	{
		_log.info("Saving ofline traders.");
		Connection con = null;
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
	...
	public static void storeTradeItems(L2PcInstance pc, PreparedStatement stm) throws SQLException
	{
		...
		{
			case L2PcInstance.STORE_PRIVATE_BUY:
				for (com.dream.game.model.TradeList.TradeItem it : pc.getBuyList().getItems())
				{
			...
			case L2PcInstance.STORE_PRIVATE_PACKAGE_SELL:
			case L2PcInstance.STORE_PRIVATE_SELL:
				for (com.dream.game.model.TradeList.TradeItem it : pc.getSellList().getItems())
				{
