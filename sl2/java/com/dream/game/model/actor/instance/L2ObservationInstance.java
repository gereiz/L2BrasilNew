...
public final class L2ObservationInstance extends L2NpcInstance
{
	...
	private final static boolean canObserve(int x, int y, int z, boolean special)
	{
		int[][] allowed = special ? ALLOWED_SPECIAL_PLACES : ALLOWED_PLACES;
		for (int[] coords : allowed)
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		if (command.startsWith("observeSiege"))
		{
			...
			if (SiegeManager.checkIfInZone(Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken())))
			{
