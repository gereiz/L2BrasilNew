...
{
	private static Logger _log = Logger.getLogger(SkillSpellbookTable.class);
	...
	private SkillSpellbookTable()
	{
		...
		File f = new File(Config.DATAPACK_ROOT + "/data/xml/player/skills/skill_spellbooks.xml");
		if (!f.exists())
		{
		...
		_log.info("Skill Spellbook: Loaded " + _skillSpellbooks.size() + " spellbooks.");
	}
	...
	public int getBookForSkill(int skillId, int level)
	{
		if (skillId == L2Skill.SKILL_DIVINE_INSPIRATION && level != -1)
		{
			switch (level)
			{
				...
				case 5:
					return 9890; // Ancient Book - Divine Inspiration (Original Version)
				default:
					return -1;
			}
		}
