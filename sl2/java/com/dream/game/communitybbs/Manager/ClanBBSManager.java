...
public class ClanBBSManager extends BaseBBSManager
{
	...
	private void clanNotice(L2PcInstance activeChar, int clanId)
	{
		...
		if (cl != null)
			...
			{
				StringBuilder html = new StringBuilder("<html><body><br><br>" + "<table border=0 width=610><tr><td width=10></td><td width=600 align=left>" + "<a action=\"bypass _bbshome\">HOME</a> &gt; " + "<a action=\"bypass _bbsclan_clanlist\"> CLAN COMMUNITY </a>  &gt; " + "<a action=\"bypass _bbsclan_clanhome;" + String.valueOf(clanId) + "\"> &amp;$802; </a>" + "</td></tr>" + "</table>");
	...
	public void parsecmd(String command, L2PcInstance activeChar)
	{
		...
		{
			if (command.startsWith("_bbsclan_clanlist"))
			{
				clanlist(activeChar, 1);
			}
			else if (command.startsWith("_bbsclan_clanlist;"))
		...
		{
			if (command.startsWith("_bbsclan_clanhome"))
			{
				clanhome(activeChar);
			}
			else if (command.startsWith("_bbsclan_clanhome;"))
